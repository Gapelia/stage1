...
//    private static final String GET_GOD_LIBRARIES = "SELECT * FROM libraries where created_by=1 order by random()";
	...
	private static final String GET_BOOKS_IN_LIBRARY = "SELECT library_books.library_id,library_books.book_id,last_updated FROM library_books join books on library_books.book_id = books.id WHERE library_books.library_id = ? ORDER BY last_updated DESC";
