...
public class Notifications {
	...
	@Path("createCommentNotification")
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	...
	@Path("getCommentNotifications")
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	public String getCommentNotifications(@FormParam("sessionId") String sessionId) {
		if (!APIUtil.isValidSession(sessionId))
			return APIUtil.INVALID_SESSION_ERROR_MSG;
		Gson gson = new GsonBuilder().create();
		User u = SessionManager.getUserFromSessionId(sessionId);
		return gson.toJson(QueryDatabaseNotifications.getCommentNotifications(u));
	}

	@Path("removeCommentNotification")
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	public String removeCommentNotification(@FormParam("sessionId") String sessionId,
											@FormParam("notificationId") int notificationId) {
		if (!APIUtil.isValidSession(sessionId))
			return APIUtil.INVALID_SESSION_ERROR_MSG;
		Gson gson = new GsonBuilder().create();

		return gson.toJson(QueryDatabaseNotifications.removeCommentNotification(notificationId));
	}
