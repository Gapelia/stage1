(function() {
	...
	var
	root = this, // Root object, this is going to be the window for now
	...
	function iterateTextMenuButtons(callback) {
		...
		var
		textMenuButtons = document.querySelectorAll(".text-menu button"),
		i,
		len,
		node;
	...
	function insertListOnSelection(sel, textProp, listType) {
		...
		var
		execListCommand = listType === "ol" ? "insertOrderedList" : "insertUnorderedList",
	...
	function getParentWithTag(node, nodeType) {
		...
		var
		checkNodeType = function(node) { return node.nodeName.toLowerCase() === nodeType; },
	...
	function getParentHref(node) {
		...
		var
		checkHref = function(node) { return typeof node.href !== "undefined"; },
	...
	function triggerTextSelection() {
		...
		var
		selectedText = root.getSelection(),
		range,
		clientRectBounds;
