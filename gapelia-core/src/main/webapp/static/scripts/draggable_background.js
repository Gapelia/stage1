...
!function($) {
	...
	$.fn.backgroundDraggable = function(options) {
		...
		return this.each(function() {
			...
			var $this = $(this), $bg = $this.css("background-image"), src = $bg.match(/^url\(['"]?(.*?)['"]?\)$/i);
			// var $article = $("article");
			...
			$this.on("mousedown.dbg touchstart.dbg", function(e) {
				...
				var x0 = e.clientX, y0 = e.clientY, pos = $this.css("background-position").match(/(-?\d+).*?\s(-?\d+)/) || [], xPos = parseInt(pos[1]) || 0, yPos = parseInt(pos[2]) || 0;
