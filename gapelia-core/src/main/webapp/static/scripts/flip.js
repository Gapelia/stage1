...
;(function ($, window, document, undefined) {
	...
	var pluginName = "flip", defaults = {
		effect: "flip",				// "flip" or "slide" // (slide will be selected for non-3d // transition supported browser)
	...
	Flip.prototype._initShadow = function () {
		...
		$flipShadow.css("position", "absolute").css("background-color", "#111").css("opacity", "0.9").css("display", "none").css("zIndex", FLIPPING_BASE_ZINDEX);

		if (this._isVertical()) {
			$flipShadow.css("width", (elemWidth) + "px").css("height", (elemHeight / 2) + "px").css("top", (elemHeight / 2) + "px").css("left", "0");
		} else {
			$flipShadow.css("width", (elemWidth / 2) + "px").css("height", elemHeight + "px").css("left", (elemWidth / 2) + "px").css("top", "0");
		}
	...
	Flip.prototype._isVertical = function () {
		return (this.options.forwardDir === DIR_TTOB || this.options.forwardDir === DIR_BTOT) ? true : false;
	}
	...
	Flip.prototype._splitElem = function ($elem, customClass) {
		...
		if ($elem === null || $elem.length === 0) {
			...
			$elem = $('<div class="splitEmpty" style="width: ' + rootWidth + 'px; height: ' + rootHeight + 'px;"></div>');
		}
		...
		if ($firstHalf.length === 0) {
			$firstHalf = $('<div class="splitHalf firstHalf ' + firstCustom + '"></div>').css('zIndex', FLIPPING_BASE_ZINDEX);
		} else {
			$firstHalf.empty();
		}
		...
		if ($secondHalf.length === 0) {
			$secondHalf = $('<div class="splitHalf secondHalf ' + secondCustom + '"></div>').css('zIndex', FLIPPING_BASE_ZINDEX);
		} else {
			$secondHalf.empty();
		}
		...
		if (isVertical) {
			$firstHalf.css("width", elemWidth + "px").css("height", Math.floor(elemHeight / 2) + "px");
			$firstHalf.children(":first").css("height", elemHeight + "px").css("display", "block");
		} else {
			$firstHalf.css("width", Math.floor(elemWidth / 2) + "px").css("height", elemHeight + "px");
			$firstHalf.children(":first").css("width", elemWidth + "px").css("display", "block");
		}

		// for second half element
		if (isVertical) {
			$secondHalf.css("width", elemWidth + "px").css("height", Math.ceil(elemHeight / 2) + "px").css("top", Math.floor(elemHeight / 2) + "px");
			$secondHalf.children(":first").css("height", elemHeight + "px").css("display", "block").css("top", -1 * Math.floor(elemHeight / 2) + "px");
		} else {
			$secondHalf.css("width", Math.ceil(elemWidth / 2) + "px").css("height", elemHeight + "px").css("left", Math.floor(elemWidth / 2) + "px");
			$secondHalf.children(":first").css("width", elemWidth + "px").css("display", "block").css("left", -1 * Math.floor(elemWidth / 2) + "px");
		}
	...
	Flip.prototype.init = function (flippingSide) {
		...
		if (this.options.forwardDir === DIR_LTOR && flippingSide === _FIRST_HALF || this.options.forwardDir === DIR_RTOL && flippingSide === _SECOND_HALF || this.options.forwardDir === DIR_TTOB && flippingSide === _FIRST_HALF || this.options.forwardDir === DIR_BTOT && flippingSide === _SECOND_HALF) {
		...
		if (this._isVertical()) {
			...
			$flippingElem.addClass("horizontalFlipping");
			$flippingBackElem.addClass("horizontalFlipping");
	...
	Flip.prototype.action = function (to) {
		...
		if (to === 0) {
			b = 0; // replace start and end so that same logic can be used
		}
		...
		var loop = function () {
			...
			if (endRatio === 0) {
				y = Math.abs(startRatio) - y; // set next ratio to the reverse direction
			}
	...
	Flip.prototype.cleanup = function () {
		...
		*/
		...
		if (this.options.forwardDir === DIR_LTOR && this.flippingSide === _FIRST_HALF || this.options.forwardDir === DIR_RTOL && this.flippingSide === _SECOND_HALF || this.options.forwardDir === DIR_TTOB && this.flippingSide === _FIRST_HALF || this.options.forwardDir === DIR_BTOT && this.flippingSide === _SECOND_HALF) {
	...
	Slide.prototype.init = function (slideSide) {
		...
		if (this.options.forwardDir === DIR_RTOL || this.options.forwardDir === DIR_LTOR) {
		...
		if (this.options.forwardDir === DIR_LTOR && slideSide === _FIRST_HALF || this.options.forwardDir === DIR_RTOL && slideSide === _SECOND_HALF || this.options.forwardDir === DIR_TTOB && slideSide === _FIRST_HALF || this.options.forwardDir === DIR_BTOT && slideSide === _SECOND_HALF) {
		...
		$sliding.css("width", $cur.width() + "px").css("height", $cur.height() + "px").css("zIndex", FLIPPING_BASE_ZINDEX);
