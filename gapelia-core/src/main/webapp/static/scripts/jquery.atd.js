...
function AtDCore() {
	...
	this.ignore_types = ['Diacritical Marks']//'Bias Language', 'Cliches', 'Complex Expression', 'Diacritical Marks', 'Double Negatives', 'Hidden Verbs', 'Jargon Language', 'Passive voice', 'Phrases to Avoid', 'Redundant Expression'];
...
AtDCore.prototype.processXML = function(responseXML) {
	...
	for (var i = 0; i < errors.length; i++) {
		if (errors[i].getElementsByTagName('string').item(0).firstChild != null) {
			...
			console.log(errors[i].getElementsByTagName('string').item(0).firstChild);
...
AtDCore.prototype.markMyWords = function(container_nodes, errors) {           
	...
	this.map(nl, function(n) {
		...
		if (n.nodeType == 3) {
			...
			console.log("replacelength: " + doReplaces.length);
