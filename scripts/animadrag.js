...
(function ($) {
	$.fn.animaDrag = function (o, callback) {
		...
		return this.each(function () {
			var id, startX, startY, draggableStartX, draggableStartY, dragging = false,
				Ev, draggable = this,
				grip = ($(this).find(o.grip).length > 0) ? $(this).find(o.grip) : $(this);
			if (o.boundary) {
				var limitTop = $(o.boundary).offset().top,
					limitLeft = $(o.boundary).offset().left,
					limitBottom = limitTop + $(o.boundary).innerHeight(),
					limitRight = limitLeft + $(o.boundary).innerWidth();
			}
				...
				id = setInterval(function () {
					if (lastX != Ev.pageX || lastY != Ev.pageY) {
						var positionX = draggableStartX - (startX - Ev.pageX),
							positionY = draggableStartY - (startY - Ev.pageY);
						...
						$(draggable).stop().animate({
							...
							o.afterEachAnimation.call(draggable, Ev)
						});
					}
					lastX = Ev.pageX;
					lastY = Ev.pageY;
			...
			$(document).mousemove(function (e) {
				if (dragging) {
					...
					// $("#jq_ez_bg").children("img").mousemove(function() { return false; });
				}
			});
