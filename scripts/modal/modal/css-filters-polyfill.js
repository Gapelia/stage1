...
;(function(window) {
	...
	var polyfilter = {
			...
			window.setTimeout(function() {
				...
				xmlHttpCheck.onreadystatechange = function() {
					if(xmlHttp.readyState == 4 && xmlHttp.status != 200) {
						alert('The configured path \r\rvar polyfilter_scriptpath = "' + window.polyfilter_scriptpath + '"\r\rseems wrong!\r\rConfigure the polyfill\'s correct absolute(!) script path before referencing the css-filters-polyfill.js, like so:\r\rvar polyfilter_scriptpath = "/js/css-filters-polyfill/";\r\rLeaving IE dead in the water is not an option, you damn Mac user... ;)');
					}
				};
			...
			for(var i = 0; i < stylesheets.length; i++) {
				(function(i) {
					switch(stylesheets[i].nodeName) {
						...
						case "STYLE": polyfilter._stylesheets.push({
						...
						case "LINK": if(stylesheets[i].rel === "stylesheet") {
		...
		_processDeclarations:	function(rule) {
			...
			for(var k in rule.declarations) {
				...
				if(declaration.property === "filter") {
					...
					var values = gluedvalues.split(/\)\s+/), properties = {
						filtersW3C: [],
						filtersWebKit: [],
						filtersSVG: [],
						filtersIE: [],
			...
			for(var i = 0; i < this._stylesheets.length; i++) {
				...
				if(sheet !== null) for(var j in sheet.cssRules) {
					...
					switch(rule.type) {
						...
						case 1: newstyles += this._processDeclarations(rule);
						break;

						case 4: newstyles += "@media " + rule.media.join(",") + "{";
		...
		convert: function(value) {
			...
			if(fmatch !== null) {
				var amount = parseInt(fmatch[2], 10), properties = this.filters.blur(amount);
			}

			// Invert
			var fmatch = value.match(/(invert)\(([0-9\.]+)\)/i);
			if(fmatch !== null) {
				var amount = parseFloat(fmatch[2], 10), properties = this.filters.invert(amount);
			}

			// Brightness
			var fmatch = value.match(/(brightness)\(([0-9\.]+)%\)/i);
			if(fmatch !== null) {
				var amount = parseFloat(fmatch[2], 10), properties = this.filters.brightness(amount);
			}

			// Drop Shadow
			var fmatch = value.match(/(drop\-shadow)\(([0-9]+)[px]*\s+([0-9]+)[px]*\s+([0-9]+)[px]*\s+([#0-9]+)\)/i);
			if(fmatch !== null) {
				var offsetX = parseInt(fmatch[2],10), offsetY = parseInt(fmatch[3],10), radius = parseInt(fmatch[4],10), color = fmatch[5], properties = this.filters.dropShadow(offsetX, offsetY, radius, color);
			}
